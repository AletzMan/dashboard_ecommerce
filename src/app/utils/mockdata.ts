...
export const YearlySalesData: IYearlySales[] = [
  ...
  },
  {
    ...
    data: [
      ...
      { month: "October", sales: 290, salesAmount: 114500 },
      { month: "November", sales: 240, salesAmount: 98000 },
      { month: "December", sales: 300, salesAmount: 225000 },
    ],
  },
  {
    year: 2024,
    data: [
      { month: "January", sales: 75, salesAmount: 45500 },
      { month: "February", sales: 0, salesAmount: 0 },
      { month: "March", sales: 0, salesAmount: 0 },
      { month: "April", sales: 0, salesAmount: 0 },
      { month: "May", sales: 0, salesAmount: 0 },
      { month: "June", sales: 0, salesAmount: 0 },
      { month: "July", sales: 0, salesAmount: 0 },
      { month: "August", sales: 0, salesAmount: 0 },
      { month: "September", sales: 0, salesAmount: 0 },
      { month: "October", sales: 0, salesAmount: 0 },
      { month: "November", sales: 0, salesAmount: 0 },
  ...
  for (let i = 0; i < numOrders; i++) {
    ...
    for (let index = 0; index < quantityProductsSelected; index++) {
      ...
      const productSelected = productsData.find(
        (product) => product.id === (random === 32 ? 31 : random)
      )
      productsSelected.push(productSelected)
    }
    const randomState = Math.floor(Math.random() * productsData.length)
    const stateSelected: IState = STATES_ARRAY.find(
      (state) => state.id === (randomState === 32 ? 31 : randomState)
    ) as IState
